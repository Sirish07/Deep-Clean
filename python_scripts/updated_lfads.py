...
#         self.output_size          = input_size if output_size is None else output_size
        ...
        self.fc_hidden = nn.Linear(in_features = self.factor_size, out_features = 50)
        self.fc_clean = nn.Linear(in_features = 50, out_features = 111)
...
#         self.register_buffer('g_posterior_logvar',None)
    ...
    def forward(self, input):
        ...
        for t in range(self.steps_size):
            ...
            hidden_out = F.relu(self.fc_hidden(factor_state))
            clean_data = self.fc_clean(hidden_out)
